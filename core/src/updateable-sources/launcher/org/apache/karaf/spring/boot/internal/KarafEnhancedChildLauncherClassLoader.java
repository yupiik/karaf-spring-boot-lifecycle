...
import org.apache.karaf.spring.boot.internal.factories.SpringFactoriesRewriter;
...
public class KarafEnhancedChildLauncherClassLoader extends LaunchedURLClassLoader {
    ...
    protected Class<?> loadClass(final String name, final boolean resolve) throws ClassNotFoundException {
        ...
        if (integrations.shouldLoadFromBundle(name)) {
            return integrations.loadFromBundle(name);
        }
    ...
    public Enumeration<URL> getResources(final String name) throws IOException {
        if ("META-INF/spring.factories".equals(name)) {
            return enumeration(Stream.concat(
                    handleSpringFactories(name),
                    Stream.of(new URL("karaf-spring-boot", null, -1, name, new StaticURLStreamHandler(
                            "org.springframework.boot.SpringApplicationRunListener=org.apache.karaf.spring.boot.services.base.SpringApplicationContextCapture"))))
                    .collect(toList()));
        }
        return super.getResources(name);
    }

    private Stream<URL> handleSpringFactories(final String name) throws IOException {
        return list(super.getResources(name)).stream()
                .flatMap(SpringFactoriesRewriter::rewrite);
    }
    ...
    public static class Dumper {
        public static void main(String[] args) throws Exception {
            ASMifier.main(new String[]{
                    "target/classes/org/apache/karaf/spring/boot/internal/KarafEnhancedChildLauncherClassLoader.class"});
            System.out.println("-------------------");
            ASMifier.main(new String[]{
                    "target/classes/org/apache/karaf/spring/boot/internal/KarafEnhancedChildLauncherClassLoader$StaticURLStreamHandler.class"});
            System.out.println("-------------------");
            ASMifier.main(new String[]{
                    "target/classes/org/apache/karaf/spring/boot/internal/KarafEnhancedChildLauncherClassLoader$StaticURLStreamHandler$StaticURLConnection.class"});
        }
    }
